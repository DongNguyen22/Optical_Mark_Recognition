...
def parse_excel_2025(sheets: dict) -> dict:
    ...
    for _, row1 in df1.iterrows():
        ma = norm_ma(row1[df1.columns[0]])
        if _blank(ma):
            continue

        # PHẦN 1: lấy tất cả cột sau cột mã
        raw1 = [
            str(row1[c]).strip().upper()
            for c in df1.columns[1:]
            if not _blank(row1[c])
        ]

        if _is_header_row(raw1):
            continue

        p1 = [MP_INT.get(v, 0) for v in raw1]

        # PHẦN 2
        r2 = df2[df2[df2.columns[0]].apply(norm_ma) == ma]
        if not len(r2):
            print(f"Không tìm thấy mã đề {ma} trong PHAN2")
            print("Các mã đề PHAN2 đang có:",
                df2[df2.columns[0]].apply(norm_ma).tolist())
            continue

        r2 = r2.iloc[0]

        raw2 = [
            str(r2[c]).strip().upper()
            for c in df2.columns[1:]
            if not _blank(r2[c])
        ]

        # mỗi câu phần 2 có 4 mệnh đề Đ/S
        p2 = []
        for i in range(0, len(raw2), 4):
            group = raw2[i:i+4]
            if len(group) == 4:
                p2.append([TF.get(v, 0) for v in group])

        # PHẦN 3
        r3 = df3[df3[df3.columns[0]].apply(norm_ma) == ma]
        if not len(r3):
            print(f"Không tìm thấy mã đề {ma} trong PHAN3")
            print("Các mã đề PHAN3 đang có:",
                df3[df3.columns[0]].apply(norm_ma).tolist())
            continue
...
def build_excel_result(sv_df: pd.DataFrame, results: list) -> bytes:
    ...
    for ri, rec in enumerate(results, 4):
        ...
        not_in_dssv = rec.get("_not_in_dssv", False)
...
def build_pdf_result(sv_df, results, loai_label) -> bytes:
    ...
    data = [hdr]
    for stt, rec in enumerate(results, start=1):
        ...
        if rec.get("_not_in_dssv"):
            ...
            for i, c in enumerate(sv_cols):
                if i == 0:          # cột STT
                    value = stt
                elif i == 1:        # cột Mã sinh viên
                    value = rec.get("sbd", "")
                else:
                    value = "NULL"

                row.append(Paragraph(str(value), N))
        ...
        note = (
            f"SBD {rec.get('sbd', '')} không có trong DS"
            if rec.get("_not_in_dssv")
            else ""
        )
...
def _grade_one(task: dict) -> dict:
    ...
    try:
        img_np = img_bytes_to_np(img_bytes)

        # 1) Đọc SBD + mã đề
        img_mdd, sbd, ma_de = run_mdd(img_np, loai)
        print("made: ",ma_de)
        print("loai: ",loai)
        # 2) Lấy đáp án theo mã đề (fallback về mã đề đầu tiên)
        ans = dap_an.get(ma_de)
        ma_de_display = ma_de
        if ans is None:
            first_key     = list(dap_an.keys())[0]
            ans           = dap_an[first_key]
            ma_de_display = first_key + " (?)"

        # 3) Chấm bài
        img_graded, score, my_index = run_omr(img_mdd, loai, ans)

        # Chuyển ảnh → bytes để truyền qua thread boundary an toàn
        pil = Image.fromarray(cv2.cvtColor(img_graded, cv2.COLOR_BGR2RGB))
        buf = io.BytesIO()
        pil.save(buf, "PNG")
        ...
        return {
            ...
            "my_index":     my_index,
            "ans":          ans,
            "img_bytes":    buf.getvalue(),   # PNG bytes
        }
...
with tab_up:
    ...
    if img_files:
        ...
        if st.session_state.get("_last_elapsed"):
            ...
            av = st.session_state["_last_avg"]
            st.markdown(
                f'<div class="bk">✓ Chấm xong <b>{n}</b> bài &nbsp;·&nbsp; '
                f'⏱ <b>{t}</b> &nbsp;·&nbsp; TB <b>{av:.2f}s</b>/bài</div>',
                unsafe_allow_html=True
            )
        ...
        max_w = st.select_slider(
            ...
            options=[1, 2, 4, 6, 8],
            value=4,
            help="Thông số mặc định là 4 ,Tăng lên nếu CPU máy có nhiều nhân hơn."
        )
        ...
        if btn and new_files and st.session_state.dap_an:
            # ── Đọc bytes trước khi vào pool (UploadedFile không thread-safe) ──
            tasks = [
                {
                    "fname":     f.name,
                    "img_bytes": f.read(),
                    "loai":      loai,
                    "dap_an":    dict(st.session_state.dap_an),  # snapshot
                }
                for f in new_files
            ]
            ...
            with ThreadPoolExecutor(max_workers=max_w) as pool:
                ...
                for fut in as_completed(future_map):
                    ...
                    prog.progress(
                        completed / len(tasks),
                        text=f"⏳ {completed}/{len(tasks)} — {res['fname']}"
                    )

                    if not res["ok"]:
                        errs.append(f"{res['fname']}: {res['error']}")
                        continue

                    # Decode ảnh PNG bytes → numpy BGR
                    img_arr = np.frombuffer(res["img_bytes"], np.uint8)
                    img_graded = cv2.imdecode(img_arr, cv2.IMREAD_COLOR)
                    with _lock:
                        st.session_state.graded_imgs[res["fname"]] = img_graded

                    # Khớp danh sách SV
                    sv_row      = {}
                    not_in_dssv = False
                    if sv_df is not None and len(sv_df) and id_col in sv_df.columns:
                        match = sv_df[sv_df[id_col].str.strip() == res["sbd"]]
                        if len(match):
                    ...
                    with _lock:
                        st.session_state.results.append({
                            **sv_row,
                            ...
                            "diem":         res["score"],
                            "_ans_key":     res["ans"],
                            "_my_index":    res["my_index"],
                            "_not_in_dssv": not_in_dssv,
                        })
             #── Sort kết quả về đúng thứ tự ảnh gốc ──────────────────────────
            original_order = [f.name for f in new_files]
            st.session_state.results.sort(
                key=lambda r: original_order.index(r["_filename"])
                              if r["_filename"] in original_order else 999
            )            
            prog.empty()
            status.empty()
            ...
            # ── Lưu thời gian vào session_state để hiện sau rerun ──
            st.session_state["_last_elapsed"] = time_str
            st.session_state["_last_count"]   = completed - len(errs)
            st.session_state["_last_avg"]      = elapsed / max(completed - len(errs), 1)

            if errs:
                with st.expander(f"⚠ Lỗi {len(errs)} file – bấm để xem chi tiết"):
                    for e in errs:
                        st.code(e)
            if completed > len(errs):
                st.markdown(
                    f'<div class="bk">✓ Chấm xong <b>{completed - len(errs)}</b> bài!'
